...
df_ = read.csv('dataframes/df_combined_all_empirical_Aug24.csv') 
...
# 521 nans. Can occur if a journal has 5+ papers with a results section in a 
#   year but none have p-values

formula = 'p_fragile_j ~ SNIP * year + log_cites_year_z * year + 
                         research * year + 
                         (1 + SNIP  | journal_clean) + (1 | school) + 
                         (1 | country)'
m = lmer(formula, data=df,  bREML=F, control=lmerControl('bobyqa'))
print(summary(m))
