...
if __name__ == '__main__':
    ...
    df = read_csv_fast(r'..\dataframes\df_combined_Aug24.csv', verbose=0)
    df = df[df['year'] >= 2004]
    df = df[df['journal'] != 'journal_of_fluorescence']  # lens.org error entry?
    ...
    all_insig = ((df['num_ps_any'] > 0) & (df['num_ps'] == 0)).sum()
    ...
    df_results['has_signif_ps'] = df_results['sig'] > 0
    ...
    df_by_p = read_csv_fast(r'..\dataframes'
                            r'\df_by_pval_combined_pruned_Aug24.csv',
                            verbose=0)
    ...
    df_empirical = read_csv_fast(
        r'..\dataframes\df_combined_all_empirical_Aug24.csv', verbose=0)
