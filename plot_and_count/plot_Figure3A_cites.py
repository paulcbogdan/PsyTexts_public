...
def do_paper_corr(key='p_fragile'):
    ...
    if key == 'p_fragile':
        plt.savefig('../figs_and_tables/Figure_3A_cites.png', dpi=600)
    elif key == 'p_fragile_implied':
        plt.savefig('../figs_and_tables/Figure_S6A_cites_implied.png', dpi=600)
    plt.show()
...
def plot_bar_continuous(df, y, key, ):
    ...
    colors = [cmap(0.925), cmap(0.925), cmap(0.925),
              cmap(0.7), cmap(0.45), cmap(0.225), cmap(0.05)]
    ...
    if key == 'p_fragile':
        ...
        plt.title('Fragile implied p-values x Citations',
                  fontsize=14, pad=12)
        plt.text(5.9, M * 1.01,
                 f'+{gain:.1%}', va='center', ha='right', fontsize=14, )
