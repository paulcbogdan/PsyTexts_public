...
def make_combined_df(all_aff=False):
    fp_p = r'../dataframes/df_p_process_Aug24.csv'
    ...
    df.drop(columns=['index', 'lens_cites', 'cnt', 'cnt_implied',
    ...
    if not all_aff:
        ...
        save_csv_pkl(df[['doi_str', 'jrl_cnt', 'country_school_count']],
                     r'../dataframes/df_journal_school_count_Aug24.csv')

    aff_all_str = '_all_aff' if all_aff else ''
    fp_out = f'../dataframes/df_combined{aff_all_str}_Aug24.csv'
    print(f'{fp_out=}')
...
def make_combined_by_pval(all_aff=False):
    df_non_p = get_non_ps_df(all_aff=all_aff)
    fp_p = r'../dataframes/df_p_processed_by_p_Aug24.csv'
    ...
    df_other = read_csv_fast(r'../dataframes/df_journal_school_count_Aug24.csv')
    df_by_p = df_by_p.merge(df_other, on='doi_str')

    aff_all_str = '_all_aff' if all_aff else ''
    fp_out = fr'../dataframes/df_by_pval_combined{aff_all_str}_Aug24.csv'
