...
def plot_jitterplot(df_y, df_n, col):
    ...
    plt.scatter(jitter + 0.5, df_y[col], alpha=.5, color='green',
                linewidth=0.5)
    ...
    if col == 'p_fragile_implied':
        ...
        plt.tight_layout()
        plt.savefig('../figs_and_tables/Figure_S5_replicability_implied_Aug24.png',
                    dpi=600)
    else:
        plt.ylabel('Fragile p-values (.01 < p < .05)', fontsize=12.5)
        plt.tight_layout()
        plt.savefig('../figs_and_tables/Figure_S1_replicability_Aug24.png', dpi=600)
    plt.show()
...
def validate_replicability(col='p_fragile'):
    fp = fr'../dataframes/df_combined_Aug24.csv'
